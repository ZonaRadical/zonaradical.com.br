class DiscourseController < ApplicationController
  def sso
      sso = SingleSignOn.parse(request.query_string, Rails.application.secrets.discourse_secret)
      if current_user.nil?
  ...
  def after_sign_in
    sso = SingleSignOn.parse(session[:discourse_qs], Rails.application.secrets.discourse_secret)
