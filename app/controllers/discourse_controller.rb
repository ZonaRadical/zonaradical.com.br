class DiscourseController < ApplicationController
  def sso
      secret = "asdasdiohreghergph34983vkjnvfjkn4qr8"
      sso = SingleSignOn.parse(request.query_string, secret)
      if current_user.nil?
        store_location_for(:user, 'http://forum.zonaradical.dev/session/sso_login')
        ...
        redirect_to sso.to_url('http://localhost:3000/session/sso_login')
      end
  end
end
